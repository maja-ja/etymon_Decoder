    ...
    if st.session_state.get('flipped'):
        st.markdown(f"""
            <div style="background-color: #E3F2FD; padding: 25px; border-radius: 15px; margin-top: 20px; border-left: 10px solid #1E88E5;">
                <p style="font-size: 2em; margin-bottom: 10px; color: #666"><b>拆解：</b> <span style="color: #D32F2F;">{q['breakdown']}</span></p>
                <p style="font-size: 1.5em; color: #333"><b>釋義：</b> {q['definition']}</p>
            </div>
        """, unsafe_allow_html=True)
